...
- (void)showSheet:(int)flg {
    NSString *localStr = NSLocalizedString(@"本地截图", nil);
    NSString *netStr = NSLocalizedString(@"远程截图", nil);
    
    if (flg == 22) {
        localStr = NSLocalizedString(@"本地视频", nil);
        netStr = NSLocalizedString(@"远程视频", nil);
    }
    UIActionSheet *showSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil otherButtonTitles:localStr,netStr, nil];
    showSheet.tag = flg;
