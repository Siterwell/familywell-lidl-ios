...
+(SceneListItemData *)ItemDataToSceneListItemData:(ItemData *)data{
    ...
    if([data.customTitle isEqualToString:@""]){
        item.custmTitle = [NSString stringWithFormat:@"%@ %@",NSLocalizedString(data.title, nil) ,data.devID];
    }else{
    item.custmTitle = data.customTitle;
    }
    item.deviceId = data.devID;
...
//        }
    ...
    for (SceneListItemData *item in outarray) {
        if (item.deviceId) {
            NSString *devieTanid = [BatterHelp gethexBybinary:[item.deviceId intValue]];
